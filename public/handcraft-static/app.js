...
function toHandcraftApiUrl(url) {
  if (typeof url !== 'string') return url;
  if (!url.startsWith('/api/')) return url;
  return `${HANDCRAFT_API_PREFIX}${url.slice('/api'.length)}`;
}
...
function bindEvents() {
  ...
  $('loginLink').addEventListener('click', openSharedWalletLogin);
  $('logoutLink').addEventListener('click', openSharedWalletLogin);
...
function updateAccountBadge() {
  ...
  $('accountBadge').textContent = isPublicAccess
    ? '未登录 · 请先登录'
    : `${state.account.username} · 创作点 ${formatCost(state.account.remaining, state.account.currency)}`;
  $('loginLink').hidden = !isPublicAccess;
  $('logoutLink').hidden = true;
  $('submitBtn').disabled = state.account.remaining <= 0;
  $('submitBtn').title = state.account.remaining <= 0 ? '请先登录账号，或补充创作点' : '';
}

function formatCost(value, currency = '¥') {
  const amount = Number(value || 0);
  const text = Number.isInteger(amount) ? String(amount) : amount.toFixed(4).replace(/0+$/, '').replace(/\.$/, '');
  if (currency === '点') return `${text}点`;
  return `${currency || '¥'}${text}`;
}

async function apiFetch(url, options = {}) {
  const response = await fetch(toHandcraftApiUrl(url), {
    ...options,
    headers: {
...
function openVideoPreview(record) {
  ...
  $('download').dataset.url = `${HANDCRAFT_API_PREFIX}/download-video?url=${encodeURIComponent(record.videoUrl)}&filename=${encodeURIComponent(filename)}`;
