...
import { isAccountRemoteKey, readAssetFoldersConfig, resolveAccountAssetFilePath } from '@/lib/account-assets';
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (imageUrl) {
      ...
      try {
        ...
        if (folderParam && filename) {
          const filePath = resolveAccountAssetFilePath(auth.account, folderParam, filename);
    ...
    if (folder && imageKey) {
      const filePath = resolveAccountAssetFilePath(auth.account, folder, imageKey);
    ...
    if (imageKey && !isAccountRemoteKey(auth.account, imageKey)) {
      ...
      for (const folderName of Object.values(folders)) {
        const filePath = resolveAccountAssetFilePath(auth.account, folderName, imageKey);
        if (filePath && fs.existsSync(filePath)) {
          fs.unlinkSync(filePath);
