...
3. 日常造型必须先归入一种明确使用情境：居家休息、城市通勤、职场工作、约会社交、休闲出游、运动行动或校园生活。日常不等于普通：每种情境都要使用不同的廓形、比例、层次、材质和一个克制记忆点，避免只写“白衬衫+黑裤子”“普通西装”“休闲装”等泛化搭配。
...
9. looks.costume 必须写清“使用情境、整体廓形、内外层、上下装、主辅色、材质、鞋履、签名细节”；looks.accessories 只保留真实会出现在画面中的配饰。高光造型还要在 description 或 costume 中说明其叙事记忆点。
10. 同一主角的全部 looks 要先做横向去重：相邻两套造型至少在廓形、层次、材质或视觉焦点中的两项存在明确差异，不能只替换颜色；同时至少继承衣橱基因中的主色/辅助色、常用轮廓或签名细节之一，保证变化与人物连续性并存。
11. 同一服装在连续场次中不得无故改变；只有时间、场合、身份、剧情节点或身体状态发生足够明显变化时才新增造型。参考设计原则，不直接复制任何品牌、秀场或参考图中的完整成衣。`;
}
...
27. 对所有人类角色，gender 不能只写标签，appearance、faceFeatures、hairstyle、makeup 和 looks 必须共同形成清楚稳定的性别表达。男性要保留男性眉骨、下颌、颈部、眉形与妆面特征；女性要保留女性颧面、下颌收束、眉眼、唇形与自然妆发特征。除非剧本明确写出中性、反串或伪装，禁止输出“男女难辨”的模糊五官和妆发。
